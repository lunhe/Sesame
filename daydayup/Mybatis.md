...
但mysql使用了mvcc的实现，可以实现在可重复读的事务级别下读写互不影响（这个操作是为啥，好神奇。看这儿：https://www.jianshu.com/p/8845ddca3b23）
mvcc涉及的概念： 当前读，快照读，隐式字段，undo日志，read view（读视图，记录当前活跃事务id，当前活跃最小事务id，下一个事务id）  RC和RR级别下，读视图的生成机制不同
